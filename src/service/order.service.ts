...
export class OrderService {
  constructor(
    ...
    private statusRepo: Repository<OrderStatus>,
  ...
  async createOrder(
    ...
    try {
      ...
      if (!user || !user.address) {
        throw new BadRequestException('El usuario no tiene una dirección registrada');
      }
...
//   if (order.status.name.toLowerCase() !== 'pendiente') {
//     throw new BadRequestException('Solo se pueden cancelar pedidos pendientes');
//   }

//   const cancelledStatus = await this.statusRepo.findOneBy({ name: 'cancelado' });
//   if (!cancelledStatus) {
//     throw new Error('Estado "cancelado" no está configurado en la base de datos');
//   }

//   order.status = cancelledStatus;
//   return this.orderRepo.save(order); // ✅ Solo se actualiza el estado
// }

async cancelOrder(id: number) {
  const order = await this.orderRepo.findOne({
    where: { id },
    relations: ['status', 'items', 'items.product'], // 👈 muy importante incluir items y productos
  });
  ...
  const cancelledStatus = await this.statusRepo.findOneBy({ name: 'cancelado' });
  if (!cancelledStatus) throw new Error('Estado "cancelado" no está configurado');
